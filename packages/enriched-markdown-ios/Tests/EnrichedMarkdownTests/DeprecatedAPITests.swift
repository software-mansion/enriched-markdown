...
final class DeprecatedAPITests: XCTestCase {
    private func resolve(@MarkdownThemeBuilder _ content: () -> MarkdownThemeGroup) -> MarkdownStyleConfig {
        MarkdownStyleConfig.resolve(layers: [MarkdownTheme(content)], traitCollection: .current)
    }
    ...
    func testElementModifierShimsForwardToNewNames() {
        let config = resolve {
            ...
            Spoiler().color(ThemeColorSpec.SemanticColor.secondary)
        }
    ...
    func testRememberMarkdownThemeStillBuildsATheme() {
        ...
        let config = MarkdownStyleConfig.resolve(layers: [theme], traitCollection: .current)
