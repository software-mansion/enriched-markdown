...
// Inner content is rendered with trailing margins disabled and the default line
// break strategy; the quote's own vertical inset provides the bottom gap, and
// per-segment margins between children still apply (allowTrailingMargin only
...
static NSArray<ENRMRenderedSegment *> *ENRMRenderBlockquoteChildren(MarkdownASTNode *node, StyleConfig *config)
{
  return ENRMRenderSegmentsFromAST(node, config, /*allowTrailingMargin*/ NO, /*allowFontScaling*/ YES,
                                   config.maxFontSizeMultiplier, NSLineBreakStrategyNone, /*blockquoteContent*/ YES);
}
...
- (void)applyBlockquoteNode:(MarkdownASTNode *)node
{
  ...
  NSArray<ENRMRenderedSegment *> *rendered = ENRMRenderBlockquoteChildren(node, self.config);
                         ...
                         pointScaleFactor:(CGFloat)pointScaleFactor
{
  ...
  NSArray<ENRMRenderedSegment *> *rendered = ENRMRenderBlockquoteChildren(node, config);
  CGFloat childrenHeight =
      ENRMMeasureSegmentsHeightViewFree(rendered, config, innerWidth, /*allowTrailingMargin*/ NO, pointScaleFactor,
                                        ENRMWritingDirectionModeFirstStrong, NSWritingDirectionNatural);
