...
For per-link native actions on iOS 17 and later, use [`linkContextMenus`](API_REFERENCE.md#linkcontextmenus). A nonempty configured menu takes precedence over `onLinkLongPress` and the system preview. Keep the callback for unconfigured links and other platforms.
