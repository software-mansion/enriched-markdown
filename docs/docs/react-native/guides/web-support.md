...
:::note
Web support applies to `EnrichedMarkdownText` only. `EnrichedMarkdownTextInput`
is native-only. See the [Feature support](/introduction/supported-features)
...
The web implementation also exports `WebMarkdownTextProps` which extends
`EnrichedMarkdownTextProps` with the web-only props above.
