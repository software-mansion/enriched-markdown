...
`Md4cFlags` decides **what the parser recognizes**, not how it looks. An option that is off does not hide an element - it means the syntax was never an element in the first place, and the markers stay in the text as you typed them. Getting one wrong therefore shows up as literal `==` or `^` in your output, not as an unstyled span.
